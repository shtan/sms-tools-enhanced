...
import stochasticTransformations as ST
import stochasticModel as SM

def analysis(inputFile='../../sounds/sax-phrase-short.wav', window='blackman', M=601, N=1024, t=-100, 
    minSineDur=0.1, nH=100, minf0=350, maxf0=700, f0et=5, harmDevSlope=0.01, stocf=0.1, outfolder = ''):
    """
    ...
    # compute the harmonic plus residual: residual will be used later for stochastic transformation
    hfreq, hmag, hphase, xr = HPR.hprModelAnal(x, fs, w, N, H, t, minSineDur, nH, minf0, maxf0, f0et, harmDevSlope)
    
    # Calculated undecimated stocEnv
    stocEnv_nodecimate = SM.stochasticModelAnal_nodecimate(xr, H, N)
    ...
    # write output sound 
    outputFile = 'output_sounds/' + outfolder + os.path.basename(inputFile)[:-4] + '_hpsModel.wav'
    ...
    return inputFile, fs, hfreq, hmag, mYst, stocEnv_nodecimate


def transformation_synthesis(inputFile, fs, hfreq, hmag, mYst, stocEnv_nodecimate, stocf = 0.1, freqScaling = np.array([0, 1.2, 2.01, 1.2, 2.679, .7, 3.146, .7]), 
    freqStretching = np.array([0, 1, 2.01, 1, 2.679, 1.5, 3.146, 1.5]), timbrePreservation = 1, 
    timeScaling = np.array([0, 0, 2.138, 2.138-1.0, 3.146, 3.146]), timbrePreservationStoc = 1,
    timbreScaling = 1., timbreScalingStoc = 1., maxFreqFrac = 1., outfolder = ''):
    """
    ...
    timbrePreservation: 1 preserves original timbre, 0 it does not, fraction scales timbre by timbreScaling
    timbrePreservationStoc: for stochastic envelope. 1 preserves original timbre, 0 does not
    timeScaling: time scaling factors, in time-value pairs
    timbreScaling: timbre scaling factor
    timbreScalingStoc: stoc envelope timbre scaling factor
    maxFreqFrac: maximum frequency to apply stochastic timbre scaling, as fraction of max frequency in DFT
    outfolder: folder to put output files
    """
    ...
    if (timbrePreservationStoc != 1):
        # timbre scaling for the stochastic envelope
        mYst = ST.stochasticTimbreScale(stocEnv_nodecimate, timbreScalingStoc, maxFreqFrac, stocf)
    ...
    # write output sound 
    outputFile = 'output_sounds/' + outfolder + '/' + os.path.basename(inputFile)[:-4] + '_hpsModelTransformation.wav'
    UF.wavwrite(y,fs, outputFile)
    
    outputFileSines = 'output_sounds/' + outfolder + '/' + os.path.basename(inputFile)[:-4] + '_hpsModelTransformation_sines.wav'
    outputFileStochastic = 'output_sounds/' + outfolder + '/' + os.path.basename(inputFile)[:-4] + '_hpsModelTransformation_stochastic.wav'
